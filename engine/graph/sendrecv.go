...
func SendRecv(res engine.RecvableRes, fn RecvFn) (map[engine.RecvableRes]map[string]*engine.Send, error) {
	...
	for k, v := range recv { // map[string]*Send
		...
		if st == nil {
			// A sender with no current payload is simply not ready yet.
			// The receiver keeps its existing field value and waits for a
			// later sender run to publish data and trigger a re-poke.
			continue
		}
