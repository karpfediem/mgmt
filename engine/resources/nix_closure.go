...
const (
	...
	// NixClosureModeRealize is accepted as a spelling alias for realise.
	NixClosureModeRealize = "realize"
)
...
func (obj *NixClosureRes) Validate() error {
	...
	case NixClosureModeRealize:
		obj.Mode = NixClosureModeRealise
	default:
		return fmt.Errorf("mode must be one of %q, %q, %q, or %q", NixClosureModeVerify, NixClosureModeSubstitute, NixClosureModeRealise, NixClosureModeRealize)
	}
...
func (obj *NixClosureRes) explicitPaths() []string {
	return nixUniqueSorted(obj.Paths)
}

func (obj *NixClosureRes) explicitDrvs() []string {
	return nixUniqueSorted(obj.Drvs)
}
...
func (obj *NixClosureRes) realiseInputs() []string {
	...
	return nixUniqueSorted(inputs)
}

func (obj *NixClosureRes) present(ctx context.Context) (bool, error) {
	present, err := obj.rootsPresent(ctx)
	if err != nil || !present || !obj.CheckContents {
		return present, err
	}
	return obj.closurePresent(ctx)
}

func (obj *NixClosureRes) rootsPresent(ctx context.Context) (bool, error) {
	...
	if result.ExitCode != 0 {
		if obj.init != nil && obj.init.Debug && obj.init.Logf != nil {
			obj.init.Logf("nix root hash query failed: %s", nixTrimForLog(result.Stderr))
		}
		return false, nil
	}
	return true, nil
...
func (obj *NixClosureRes) closurePresent(ctx context.Context) (bool, error) {
	...
	if result.ExitCode != 0 {
		if obj.init != nil && obj.init.Debug && obj.init.Logf != nil {
			obj.init.Logf("nix closure query failed: %s", nixTrimForLog(result.Stderr))
		}
		return false, nil
	}

	closure := nixParseLines(result.Stdout)
	if len(closure) == 0 {
		return false, nil
	}
	if obj.CheckContents {
		for _, chunk := range nixChunks(closure, 256) {
			...
			if verify.ExitCode != 0 {
				if obj.init != nil && obj.init.Debug && obj.init.Logf != nil {
					obj.init.Logf("nix verify-path failed: %s", nixTrimForLog(verify.Stderr))
				}
				return false, nil
			}
		}
...
func (obj *NixClosureRes) queryDrvOutputs(ctx context.Context, drv string) ([]string, bool, error) {
	...
	if result.ExitCode != 0 {
		if obj.init != nil && obj.init.Debug && obj.init.Logf != nil {
			obj.init.Logf("nix drv output query failed for %s: %s", drv, nixTrimForLog(result.Stderr))
		}
		return nil, false, nil
	}
	outputs := nixParseLines(result.Stdout)
...
func (obj *NixClosureRes) realise(ctx context.Context) error {
	...
	if obj.init != nil && obj.init.Debug && obj.init.Logf != nil {
		if result.Stdout != "" {
			obj.init.Logf("nix-store stdout: %s", nixTrimForLog(result.Stdout))
		}
		if result.Stderr != "" {
			obj.init.Logf("nix-store stderr: %s", nixTrimForLog(result.Stderr))
		}
	}
	if result.ExitCode != 0 {
		return fmt.Errorf("nix-store --realise failed with exit code %d: %s", result.ExitCode, nixTrimForLog(result.Stderr))
	}
	return nil
}
...
func (obj *NixClosureRes) runNix(ctx context.Context, args []string) (*nixRunResult, error) {
	...
	cmd.Env = nixMergeEnv(os.Environ(), obj.Env)
...
func nixUniqueSorted(in []string) []string {
	if len(in) == 0 {
		return nil
	}
...
func nixParseLines(value string) []string {
	return nixUniqueSorted(strings.Split(value, "\n"))
}

func nixChunks(in []string, size int) [][]string {
	if size <= 0 {
...
func nixMergeEnv(base []string, overrides map[string]string) []string {
	if len(overrides) == 0 {
		return base
	}
...
func nixTrimForLog(value string) string {
