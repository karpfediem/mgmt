...
func (obj *NixGCRootRes) Validate() error {
	...
	if !nixPathUnderDir(path, obj.GCRootsDir) {
...
func (obj *NixGCRootRes) stateOK() (bool, error) {
	...
	targetExists, err := obj.targetExists()
	if err != nil {
		return false, err
	}
	if !targetExists {
...
func (obj *NixGCRootRes) applyExists() error {
	...
	if !targetExists {
		return fmt.Errorf("target does not exist: %s", obj.Target)
	}
...
func (obj *NixGCRootRes) isStorePath(path string) bool {
	...
	return nixPathUnderDir(path, obj.StoreDir)
}

func (obj *NixGCRootRes) targetExists() (bool, error) {
	_, err := os.Stat(obj.Target)
	if err == nil {
		return true, nil
	}
	if os.IsNotExist(err) {
...
func nixPathUnderDir(path string, dir string) bool {
