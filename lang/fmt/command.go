...
func (obj *Command) Run(ctx context.Context, args []string) error {
	...
	if options.writeInPlace {
		...
		for _, path := range options.paths {
			if err := obj.rewriteFile(path); err != nil {
				return err
			}
		}
		return nil
	...
	if options.checkOnly {
		return obj.runCheck(options.paths)
	}
	...
	if len(options.paths) == 0 {
		...
		return obj.writeFormattedOutput("<stdin>", src)
	}

	if len(options.paths) > 1 {
		return fmt.Errorf("multiple files require --write or --check")
	}
	...
	return obj.writeFormattedOutput(options.paths[0], src)
}
...
func (obj *Command) runCheck(paths []string) error {
	changed := false
	if len(paths) == 0 {
		src, err := io.ReadAll(obj.stdin())
		if err != nil {
		...
		differs, err := obj.checkFormatted("<stdin>", src)
		if err != nil {
			return err
		}
		if differs {
	...
	for _, path := range paths {
		...
		differs, err := obj.checkFormatted(path, src)
		if err != nil {
			return err
		}
...
func (obj *Command) rewriteFile(path string) error {
	src, err := obj.readFile(path)
	if err != nil {
		return err
	}
	formatted, err := obj.formatSource(path, src)
	if err != nil {
		return err
	}
...
func (obj *Command) formatSource(name string, src []byte) ([]byte, error) {
	...
	return frontendformat.Document(doc)
}

func (obj *Command) writeFormattedOutput(name string, src []byte) error {
	formatted, err := obj.formatSource(name, src)
	if err != nil {
		return err
	}
...
func (obj *Command) checkFormatted(name string, src []byte) (bool, error) {
	formatted, err := obj.formatSource(name, src)
	if err != nil {
		return false, err
	}
